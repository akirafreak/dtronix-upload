...
var Roar = new Class({
    ...
    alert: function(title, message, options) {
        var items = [new Element("h3", {"html": [title, ""].pick()})];
        if (message) items.push(new Element("p", {"html": message}));
        return this.inject(items, options, message);
    },

    inject: function(elements, options, message) {
        if (!this.body) this.render();
        var options = Object.merge(Object.clone(this.options), options || {});
		
		switch(options.bgcolor){
			case "red": options.bgcolor = "#ad0000"; break;
			case "green": options.bgcolor = "#00ad01"; break;
			case "blue": options.bgcolor = "#0078ad"; break;
		}
		
		if(options.duration == "auto"){
			options.duration = Math.max(65 * message.length, 2000);
		}
        ...
        if (last) {
            ...
            offset[1] = offset[0] + last.offsetHeight + options.offset;
        }
        var to = {"opacity": 1};
        ...
        if(options.duration) {
			this.remove.delay(options.duration, this, [item]);
            var over = false;
            var trigger = (function() {
                trigger = null;
                if (!over) this.remove();
            }).delay(options.duration, this, [item]);
            item.addEvents({
                mouseover: function() {
                    over = true;
                }.bind(this),
